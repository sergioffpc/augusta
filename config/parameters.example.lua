-- The simulation's Parameters (ADR-0039): every value that client and server
-- must agree on. Only the server reads this file; each client is sent the
-- result when it joins. Copy it next to augustad.yaml and name it there with
-- the `parameters` key.
--
...
-- fixed for as long as the server runs. NFR-01 asks the server to sustain 60 Hz;
-- a slower rate is accepted so a run can be debugged.
local tick_rate_hz = 60
