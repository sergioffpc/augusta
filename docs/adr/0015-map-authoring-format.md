...
A map is authored as a **scenario**: a folder under `authoring/` holding the stage and the Lua scripts that go with it (`authoring/test_map/test_map.usda`, `authoring/test_map/parameters.lua`, ADR-0039). The cooker is told the folder and packs everything under it (ADR-0030).
