...
See ADR-0031 for the pack's actual byte layout (header/TOC/trailer) and the per-USD-prim conversion mapping.

## Considered Options

Hashing each cooked asset individually (embedding per-asset hashes in the pack's table of contents, in addition to the pack-level hash/signature) was considered, for finer-grained corruption detection than an all-or-nothing pack check. Deferred, not rejected outright: it's a compatible future addition to the pack's TOC format (ADR-0018 already treats the TOC as an implementation detail) that doesn't change this pipeline's stage order, so it's left out until a concrete need (e.g., partial pack streaming) justifies the added format complexity.
