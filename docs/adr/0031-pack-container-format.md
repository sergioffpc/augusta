...
- Character list (ADR-0042): the manifest's `characters` paths, in manifest order, as one blob at the fixed path `Characters` - a count then each path as a string, at most 255. Written to both the client and server pack: a character index names a position in it on both sides.
- Character eye (ADR-0040): the origin of a character's `Character/Eye` prim, in that character's root space, as three floats and nothing else, one blob at `<character path>/Character/Eye`. Written to the client pack only: it places the local player's camera, which the server has no use for yet.
- Client pack hash (ADR-0038): the BLAKE3 hash of the client pack cooked in the same run, its trailer's 32 bytes and nothing else, as one blob at the fixed path `ClientPack`. Written to the server pack only: the server admits only a client whose Join request names that hash, so a client never plays against a server pack cooked from other content, even one signed by the same key.
...
3. **Index** — one entry per blob: type tag (Mesh/Texture/Audio/Collision/SpawnPoint/Hitbox/Scene/Script/Characters/ClientPack/Eye), path, offset, size. Written after the data section, since it needs the recorded offsets.
