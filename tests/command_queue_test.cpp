...
TEST(ScreenTest, AcceptsAWellFormedNewerCommand) {
  EXPECT_TRUE(Screen(Walk(5), 4).has_value());
  EXPECT_TRUE(Screen(Walk(1), 0).has_value());
}

TEST(ScreenTest, RejectsASequenceThatIsNotStrictlyNewer) {
  EXPECT_EQ(Screen(Walk(4), 4).error(), Rejection::kStale);
  EXPECT_EQ(Screen(Walk(3), 4).error(), Rejection::kStale);
  EXPECT_EQ(Screen(Walk(0), 0).error(), Rejection::kStale);
}

TEST(ScreenTest, RejectsNonFiniteNumbersWhereverTheyAre) {
  SequencedCommand command = Walk(1);
  command.command.movement.direction.y = kNaN;
  EXPECT_EQ(Screen(command, 0).error(), Rejection::kNonFinite);
...
TEST(ScreenTest, RejectsNumbersOutsideWhatAClientCanProduce) {
  EXPECT_EQ(Screen(Walk(1, kMaxMovementMagnitude + 0.5F), 0).error(), Rejection::kOutOfRange);
  EXPECT_EQ(Screen(Walk(1, 1.0e30F), 0).error(), Rejection::kOutOfRange);

  SequencedCommand command = Walk(1);
  command.command.pitch = kMaxPitch + 0.1F;
  EXPECT_EQ(Screen(command, 0).error(), Rejection::kOutOfRange);
  command.command.pitch = -kMaxPitch - 0.1F;
  EXPECT_EQ(Screen(command, 0).error(), Rejection::kOutOfRange);
...
TEST(ScreenTest, AcceptsADiagonalAndALookStraightUp) {
  SequencedCommand command = Walk(1);
  command.command.movement.direction = Vec3(1.0F, 0.0F, 1.0F);
  command.command.pitch = 1.5707F;

  EXPECT_TRUE(Screen(command, 0).has_value());
}

TEST(ScreenTest, EveryRejectionHasADescription) {
...
TEST(CommandQueueTest, HandsOutOneCommandPerTickOldestFirst) {
  CommandQueue queue;
  ASSERT_TRUE(queue.Offer(Walk(1, 1.0F)).has_value());
  ASSERT_TRUE(queue.Offer(Walk(2, 0.5F)).has_value());
...
TEST(CommandQueueTest, ARepeatedCommandIsTakenInOnce) {
  CommandQueue queue;
  ASSERT_TRUE(queue.Offer(Walk(1)).has_value());

  EXPECT_EQ(queue.Offer(Walk(1)).error(), Rejection::kStale);
...
TEST(CommandQueueTest, ACommandOlderThanOneAlreadyTakenInIsDropped) {
  CommandQueue queue;
  ASSERT_TRUE(queue.Offer(Walk(5)).has_value());

  EXPECT_EQ(queue.Offer(Walk(3)).error(), Rejection::kStale);
}
...
TEST(CommandQueueTest, ADroppedCommandDoesNotAdvanceTheSequenceOrTheAcknowledgement) {
  ...
  EXPECT_EQ(queue.Offer(bad).error(), Rejection::kNonFinite);

  // Sequence 2 is still available for a valid command.
  EXPECT_TRUE(queue.Offer(Walk(2)).has_value());
}
...
TEST(CommandQueueTest, HoldsTheLastMovementForABoundThenStopsAndKeepsTheStance) {
  ...
  ASSERT_TRUE(queue.Offer(crouch).has_value());
  static_cast<void>(queue.Next());
...
TEST(CommandQueueTest, ANewCommandRestartsTheHold) {
  ...
  ASSERT_TRUE(queue.Offer(Walk(2)).has_value());
  static_cast<void>(queue.Next());
...
TEST(CommandQueueTest, HeldAndIdleTicksNeverRepeatAOneShotAction) {
  ...
  ASSERT_TRUE(queue.Offer(shoot).has_value());
...
TEST(CommandQueueTest, WhenAClientRunsAheadTheOldestCommandsGo) {
  ...
  for (std::uint32_t sequence = 1; sequence <= total; ++sequence) {
    ASSERT_TRUE(queue.Offer(Walk(sequence)).has_value());
  }
