...
TEST(ReplicationTest, PlayerAndSessionNamesConvertBothWays) {
  EXPECT_EQ(SessionOf(PlayerOf(static_cast<SessionIdWire>(77))), static_cast<SessionIdWire>(77));
...
TEST(ReplicationTest, EveryRecipientGetsEveryPlayerUnderTheNamesClientsKnow) {
  ...
  const std::array<Recipient, 2> recipients = {Recipient{.session = static_cast<SessionIdWire>(1)},
                                               Recipient{.session = static_cast<SessionIdWire>(2)}};
  ...
  for (const auto& update : updates) {
    ...
    EXPECT_EQ(update.players[0].session, static_cast<SessionIdWire>(1));
    EXPECT_EQ(update.players[0].body.position.x, 10.0F);
    EXPECT_EQ(update.players[1].session, static_cast<SessionIdWire>(2));
...
TEST(ReplicationTest, EachRecipientGetsItsOwnAcknowledgedSequence) {
  ...
  const std::array<Recipient, 2> recipients = {
      Recipient{.session = static_cast<SessionIdWire>(1), .acknowledged_sequence = 100},
      Recipient{.session = static_cast<SessionIdWire>(2), .acknowledged_sequence = 7}};

  const auto updates = PlanUpdates(state, 1, recipients);

  EXPECT_EQ(updates[0].recipient, static_cast<SessionIdWire>(1));
  EXPECT_EQ(updates[0].acknowledged_sequence, 100U);
  EXPECT_EQ(updates[1].recipient, static_cast<SessionIdWire>(2));
...
TEST(ReplicationTest, ARecipientWithNoBodyYetStillSeesTheOthers) {
  ...
  const std::array<Recipient, 1> recipients = {Recipient{.session = static_cast<SessionIdWire>(2)}};

  const auto updates = PlanUpdates(state, 1, recipients);

  ASSERT_EQ(updates.size(), 1U);
  ASSERT_EQ(updates[0].players.size(), 1U);
  EXPECT_EQ(updates[0].players[0].session, static_cast<SessionIdWire>(1));
}
